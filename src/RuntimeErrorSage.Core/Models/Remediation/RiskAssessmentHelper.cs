...
namespace RuntimeErrorSage.Core.Models.Remediation
{
    ...
    public static class RiskAssessmentHelper
    {
        ...
        public static RemediationRiskLevel CalculateRiskLevel(RemediationSeverity severity, RemediationActionImpactScope impactScope)
        {
            // Higher severity and wider scope means higher risk
            if (severity == RemediationSeverity.Critical && impactScope >= RemediationActionImpactScope.System)
            {
                return RemediationRiskLevel.Critical;
            }
            else if (severity >= RemediationSeverity.High && impactScope >= RemediationActionImpactScope.Service)
            {
                return RemediationRiskLevel.High;
            }
            else if (severity >= RemediationSeverity.Medium || impactScope >= RemediationActionImpactScope.Module)
            {
                return RemediationRiskLevel.Medium;
            }
            else if (severity >= RemediationSeverity.Low)
            {
